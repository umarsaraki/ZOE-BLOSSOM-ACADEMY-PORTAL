...
// Without this, Express thinks every request is plain HTTP, so a
// "secure" session cookie never gets set/read correctly and every user
// appears logged out on their very next request.
...
app.use(session({
  store: new SQLiteStore({ db: 'sessions.db', dir: path.join(__dirname, 'data'), ttl: SESSION_MAX_AGE_MS / 1000 }),
  secret: process.env.SESSION_SECRET || 'change_this_secret',
  resave: false,
  saveUninitialized: false,
  rolling: true, // reset the expiry countdown on every request, so active use never logs someone out
  cookie: {
    httpOnly: true,
...
// forwarded to the global error handler instead of hanging the request
...
// ---------------------------------------------------------------------
// Fee calculation helpers — compare what a student has paid against
// what their class is expected to pay (from fee_structure), so payments
// can be correctly marked PAID / PARTIAL / UNPAID instead of blindly
// stamping everything as PAID.
// ---------------------------------------------------------------------
function getExpectedFee(class_name, term, session) {
  if (!class_name) return 0;
  const row = db.prepare(
    'SELECT COALESCE(SUM(amount),0) AS total FROM fee_structure WHERE class_name=? AND term=? AND session=?'
  ).get(class_name, term, session);
  return row.total;
}
function getTotalPaid(reg_no, term, session) {
  const row = db.prepare(
    'SELECT COALESCE(SUM(amount),0) AS total FROM payments WHERE student_reg_no=? AND term=? AND session=?'
  ).get(reg_no, term, session);
  return row.total;
}
function computeFeeStatus(paid, expected) {
  if (expected <= 0) return paid > 0 ? 'PAID' : 'UNPAID'; // no fee set for this class yet
...
app.post('/api/admin/students', requireRole('admin'), asyncHandler(async (req, res) => {
...
// Bulk add students — body: { students: [{reg_no, full_name, class_name, password}, ...] }
app.post('/api/admin/students/bulk', requireRole('admin'), asyncHandler(async (req, res) => {
  ...
  for (const s of students) {
    if (!s.reg_no || !s.full_name || !s.password) {
      errors.push(`Skipped a row missing Reg No, Name, or Password.`);
      continue;
    }
    try {
      const hash = await bcrypt.hash(String(s.password), 10);
      db.prepare(
        `INSERT INTO users (role, reg_no, full_name, class_name, password_hash, status)
         VALUES ('student', ?, ?, ?, ?, 'active')`
      ).run(String(s.reg_no).trim(), s.full_name, s.class_name || null, hash);
      added++;
    } catch (err) {
      errors.push(`${s.reg_no}: ${err.message.includes('UNIQUE') ? 'already exists' : 'failed'}`);
    }
  }
  res.json({ ok: true, added, total: students.length, errors });
}));

app.put('/api/admin/students/:id', requireRole('admin'), asyncHandler(async (req, res) => {
...
app.post('/api/admin/teachers', requireRole('admin'), asyncHandler(async (req, res) => {
...
// Bulk add teachers — body: { teachers: [{reg_no, full_name, subject, password}, ...] }
app.post('/api/admin/teachers/bulk', requireRole('admin'), asyncHandler(async (req, res) => {
  ...
  for (const t of teachers) {
    if (!t.reg_no || !t.full_name || !t.password) {
      errors.push(`Skipped a row missing Reg No, Name, or Password.`);
      continue;
    }
    try {
      ...
      db.prepare(
        `INSERT INTO users (role, reg_no, full_name, subject, password_hash, status)
         VALUES ('teacher', ?, ?, ?, ?, 'active')`
      ).run(String(t.reg_no).trim(), t.full_name, t.subject || null, hash);
      added++;
    } catch (err) {
      errors.push(`${t.reg_no}: ${err.message.includes('UNIQUE') ? 'already exists' : 'failed'}`);
    }
  }
  res.json({ ok: true, added, total: teachers.length, errors });
}));

app.put('/api/admin/teachers/:id', requireRole('admin'), asyncHandler(async (req, res) => {
...
// Create the standard Nigerian school class structure in one click
app.post('/api/admin/classes/setup-standard', requireRole('admin'), (req, res) => {
  ...
  let added = 0;
  standardClasses.forEach(name => {
    try {
      db.prepare('INSERT INTO classes (class_name) VALUES (?)').run(name);
      added++;
    } catch (err) { /* already exists, skip */ }
  });
...
// Who owes what, for the current term/session — every student, with
// expected fee (from fee_structure), total paid, balance, and status.
app.get('/api/admin/fees/status', requireRole('admin'), (req, res) => {
  const settings = db.prepare('SELECT * FROM settings WHERE id=1').get();
  const term = settings.current_term;
  const session = settings.current_session;
  const students = db.prepare("SELECT reg_no, full_name, class_name FROM users WHERE role='student' ORDER BY full_name").all();

  const rows = students.map(s => {
    const totalPaid = getTotalPaid(s.reg_no, term, session);
    const expected = getExpectedFee(s.class_name, term, session);
    const balance = Math.max(0, expected - totalPaid);
    const status = computeFeeStatus(totalPaid, expected);
    return { reg_no: s.reg_no, full_name: s.full_name, class_name: s.class_name, expected, totalPaid, balance, status };
  });

  const totals = rows.reduce((acc, r) => {
    acc.totalExpected += r.expected; acc.totalPaid += r.totalPaid; acc.totalOutstanding += r.balance;
    return acc;
  }, { totalExpected: 0, totalPaid: 0, totalOutstanding: 0 });

  res.json({ rows, term, session, ...totals });
});
...
app.post('/api/admin/fees/payments', requireRole('admin'), (req, res) => {
  ...
  if (!finalStatus) {
    const student = db.prepare('SELECT class_name FROM users WHERE reg_no=?').get(student_reg_no);
    const paidBefore = getTotalPaid(student_reg_no, term, session);
    const expected = getExpectedFee(student ? student.class_name : null, term, session);
    finalStatus = computeFeeStatus(paidBefore + Number(amount), expected);
  }

  db.prepare('INSERT INTO payments (student_reg_no, amount, fee_type, term, session, status) VALUES (?,?,?,?,?,?)')
    .run(student_reg_no, amount, fee_type || 'Tuition', term, session, finalStatus);
...
app.get('/api/student/fees', requireRole('student'), (req, res) => {
  ...
  const settings = db.prepare('SELECT * FROM settings WHERE id=1').get();
  const student = db.prepare('SELECT class_name FROM users WHERE reg_no=?').get(regNo);
  const term = settings.current_term;
  const session = settings.current_session;

  const totalPaid = getTotalPaid(regNo, term, session);
  const expected = getExpectedFee(student ? student.class_name : null, term, session);
  const balance = Math.max(0, expected - totalPaid);
  const status = computeFeeStatus(totalPaid, expected);

  res.json({ account, history, totalPaid, expected, balance, status, term, session });
});

// Flutterwave: create a dedicated virtual account for a student (admin-triggered)
app.post('/api/admin/students/:regNo/create-virtual-account', requireRole('admin'), asyncHandler(async (req, res) => {
...
// Flutterwave webhook
app.post('/flutterwave/webhook', express.json(), asyncHandler(async (req, res) => {
  ...
  const student = db.prepare('SELECT class_name FROM users WHERE reg_no=?').get(account.student_reg_no);
  const settings = db.prepare('SELECT * FROM settings WHERE id=1').get();
  const term = settings.current_term;
  const session = settings.current_session;

  const paidBefore = getTotalPaid(account.student_reg_no, term, session);
  const expected = getExpectedFee(student ? student.class_name : null, term, session);
  const status = computeFeeStatus(paidBefore + amount, expected);

  db.prepare('INSERT INTO payments (student_reg_no, amount, fee_type, term, session, status, flutterwave_ref) VALUES (?,?,?,?,?,?,?)')
    .run(account.student_reg_no, amount, 'Tuition', term, session, status, data.flw_ref || data.tx_ref || null);
...
// Global error handler — always return JSON with the real error message,
// so the frontend never has to show a generic, unhelpful "Something went
// wrong" with no way to diagnose it.
